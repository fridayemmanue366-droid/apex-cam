...
def generate_photo(input_bytes: bytes, prompt: str | None = None,
                   reference_bytes: bytes | None = None) -> bytes:
    """Face-swap a single uploaded photo into the persona. Returns image bytes.
    Synchronous (Decart returns the image directly). Raises on failure."""
    import requests

    key = _load_key()
    if not key:
        raise RuntimeError("Decart key not configured")
    if reference_bytes is None and REFERENCE_IMG.exists():
        reference_bytes = REFERENCE_IMG.read_bytes()
    ...
    r = requests.post(
        ...
        files=files,
        data={"prompt": prompt or FACE_PROMPT},
        timeout=(10, 90),
    )
