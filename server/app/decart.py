...
video/restyle are jobs (submit -> poll -> /content); live cam does a WS
...
def submit_job(model: str, video_bytes: bytes, prompt: str | None,
    ...
    if reference_bytes:
        # restyle's reference is a style source; plain video's is a face — same
        # field, different meaning per model.
        files["reference_image"] = ("ref.jpg", reference_bytes, "image/jpeg")
